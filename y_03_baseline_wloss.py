...
USE_LOG_TRANSFORM = False  # Set to True to enable log transformation
...
# Modified train_epoch function to use weighted loss
def train_epoch_weighted(model, train_loader, optimizer, criterion, device, 
                        use_magnitude_weights=True, weight_power=0.5):
    """Train with magnitude-based sample weighting."""
    model.train()
    total_loss = 0
    num_batches = 0
    ...
    for batch in train_loader:
        ...
        if use_magnitude_weights and isinstance(criterion, nn.MSELoss):
            # Calculate sample weights based on target magnitude
            # Using L2 norm of target vector for each sample
            target_magnitudes = torch.norm(targets, p=2, dim=1, keepdim=True)
            weights = torch.pow(target_magnitudes + 1.0, weight_power)
            weights = weights / weights.mean()  # Normalize
            
            # Weighted loss
            mse = (predictions - targets) ** 2
            weighted_mse = mse * weights.expand_as(mse)
            loss = weighted_mse.mean()
        else:
            # Use the provided criterion (could be a custom weighted loss)
...
def train_gatv2_gnn(output_dir='results/GATv2_results'):
    print(f"\nUsing FPGA GNN model with pre-split data from {image_data_path}")
    ...
    for epoch in tqdm(range(NUM_EPOCHS), desc="Epochs", unit="epoch", disable=True):
        # Training
        train_loss = train_epoch_weighted(model, train_loader, optimizer, criterion, device)
        ...
        # ******** ADDED CODE: Calculate and save metrics/plots every 50 epochs ********
        if (epoch + 1) % 50 == 0 and epoch < NUM_EPOCHS -1 : # Avoid double plotting at the end
            print(f"\n--- Generating intermediate metrics and plots for epoch {epoch+1} ---")
            # Ensure the model is in evaluation mode
            model.eval()
            
            # Intermediate Test set evaluation
            interim_test_loss_norm, interim_test_pred, interim_test_targets = evaluate(
                model, test_loader, criterion, device, dataset, denormalize=True
            )
            interim_test_metrics = calculate_metrics(interim_test_pred, interim_test_targets, feature_names)

            # Display interim test metrics
            print(f"Interim Test Set Metrics (denormalized) at epoch {epoch+1}:")
            print(f"Overall Metrics:")
            for metric, value in interim_test_metrics['overall'].items():
                if metric in ['MAPE', 'SMAPE']:
                    print(f"  {metric}: {value:.2f}%")
                else:
                    print(f"  {metric}: {value:.4f}")
            
            # Save interim metrics to a uniquely named file
            folder_base = SAVE_FOLDERNAME.split('/', 1)[1] if '/' in SAVE_FOLDERNAME else SAVE_FOLDERNAME
            interim_metrics_filename = f"{folder_base}_epoch_{epoch+1}"
            
            # Create a specific subdirectory for interim plots to keep things organized
            interim_plots_output_dir = os.path.join(output_dir, f"epoch_{epoch+1}_plots")
            os.makedirs(interim_plots_output_dir, exist_ok=True)
            ...
            training_config_for_interim_file = {
                'batch_size': BATCH_SIZE,
            ...
            save_metrics_to_file(
                metrics=interim_test_metrics,
                output_dir=output_dir, # Save in the main output_dir, filename will be unique
                model_config=model_config_for_file,
                training_config=training_config_for_interim_file,
    ...
    # 9. Save final model
    final_model_path = f'{output_dir}/final_simple_gnn_model.pth'
    torch.save({
